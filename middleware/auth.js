export const verifyToken = (req, res, next) => {
    ...
    if (!token) {
        console.error("Erreur : Aucun token trouvé dans les cookies.");
    ...
    jwt.verify(token, env.token, (err, user) => {
        if (err) {
            if (err.name === 'TokenExpiredError') {
                console.error("Erreur : Le token a expiré.");
                return next(createError(401, "Le token a expiré"));
            }
            console.error("Erreur : Token invalide ou problème de vérification.", err);
            return next(createError(403, "Token non valide"));
        }

        console.log("Token valide. Payload extrait :", user);
        req.user = user;
        next();
    });
};
